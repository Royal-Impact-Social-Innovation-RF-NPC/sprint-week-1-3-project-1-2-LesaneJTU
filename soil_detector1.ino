...
void loop()
{
  show_monitor(read_soil(), read_temp(), read_light(),read_humid(read_temp(), read_light()));
...
int read_humid(int T, int L){
  if(T > 30 && T < 45){
    if(L>5 && L<450){
      ...
      float relative_humidity = 100*(exp((17.625*vapour)/(243.04+vapour))/exp((17.625*T)/(243.04+T))); //RH: =100*(EXP((17.625*TD)/(243.04+TD))/EXP((17.625*T)/(243.04+T)))
